...
class SignatureProcessor(SubCommand):
    ...
    async def run(self):
        ...
        async with contextlib.AsyncExitStack() as aes:
            self.psql: asyncpg.Connection = await aes.enter_async_context(pool.acquire())
            ...
            for name, factory_function in self.config.tses.all_factories():
                tse = TSEWrapper(
                    name=name,
                    factory_function=functools.partial(factory_function, name=name)
                )
                tse.start(pool)
    ...
    async def handle_hook(self, payload):
        del payload  # unused
        LOGGER.info(f"tse_signature hook")
